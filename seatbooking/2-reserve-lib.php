...
function save($sessid, $userid, $seats)
{
    ...
    foreach ($seats as $seat) {
        // Extract the seat identifier from the HTML markup
        $dom = new DOMDocument();
        $dom->loadHTML($seat);
        $seatId = $dom->getElementsByTagName('span')[0]->textContent;

        $sql .= "(?,?,?),";
        array_push($data, $sessid, $seatId, $userid);
    }
